...
public class LocationSimulator implements Runnable{
    ...
    //这里没做autowired
    //因为location simulator并不是spring Manage的一个bean
    //它只是一个线程 起起来 做任务就可以了
    //所以要通过external的setter set进来
     ...
     */
    //有一个起始时间 按照report的interval 去report and move当前的location
    //计算下一个interval要走的距离
    void moveRunningLocation() {
        ...
        for (int i = positionInfo.getLeg().getId(); i < legs.size(); i++) {
            ...
            if (Double.doubleToRawLongBits(excess) == 0) {
                ...
                //有起点坐标 move距离 heading方向 可以根据公式算出新的点的坐标
                //command+左键查看公式
